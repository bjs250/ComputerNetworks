...
class Switch(StpSwitch):
    ...
    def process_message(self, message):
        ...
        if message.root == self.root and (message.distance+1) > self.distance and message.pathThrough == True:
            ...
            if self.debugMode == True:
                print("update " + str(self.switchID) + " (farther) from " + str(message.origin), self.switchID, self.root, self.distance, self.active_links, self.switchthrough)
        ...
        """
        if message.root == self.root and (message.distance+1) > self.distance and message.pathThrough == False and message.origin in self.active_links:
